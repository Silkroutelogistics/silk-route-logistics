...
const DETENTION = { type: "DETENTION", amount: 250, quantity: 5 };
...
describe("customerPriceFor", () => {
  ...
  it("bills the negotiated rate PER UNIT when the row carries a quantity", () => {
    ...
    expect(customerPriceFor(DETENTION, { DETENTION: 75 })).toBe(375);
  });
  ...
  it("treats a quantity of ZERO as a real quantity, not as absent", () => {
    ...
    expect(customerPriceFor({ type: "DETENTION", amount: 0, quantity: 0 }, { DETENTION: 75 })).toBe(0);
  });
  ...
  it("lets an explicit customerAmount win over both the rate card and cost", () => {
    ...
    expect(customerPriceFor({ ...DETENTION, customerAmount: 400 }, { DETENTION: 75 })).toBe(400);
  ...
  it("honours an explicit customerAmount of zero — a waived charge stays waived", () => {
    ...
    expect(customerPriceFor({ ...DETENTION, customerAmount: 0 }, { DETENTION: 75 })).toBe(0);
  });
  ...
  it("falls back rather than billing nonsense when a rate card is malformed", () => {
    ...
    expect(customerPriceFor(DETENTION, { DETENTION: "75" } as any)).toBe(250);
    expect(customerPriceFor(DETENTION, { DETENTION: -10 })).toBe(250);
    expect(customerPriceFor(DETENTION, { DETENTION: NaN })).toBe(250);
  });
  ...
  it("rounds to cents", () => {
    ...
    expect(customerPriceFor({ type: "DETENTION", amount: 10, quantity: 3 }, { DETENTION: 33.333 })).toBe(100);
  });
  ...
  it("never lets the carrier-pay figure leak into the customer price when a rate exists", () => {
    ...
    const cheap = customerPriceFor({ type: "DETENTION", amount: 250, quantity: 5 }, { DETENTION: 75 });
    const dear = customerPriceFor({ type: "DETENTION", amount: 999, quantity: 5 }, { DETENTION: 75 });
