 ...
 *
 * docs/legal/bca-content-F10.md is the authoring source; the compiled constant
...
describe("the compiled BCA body matches its markdown source", () => {
  it("every field round-trips", () => {
    ...
    expect(
      ...
      "the committed constant no longer matches docs/legal/bca-content-F10.md. Run " +
        "`npx tsx scripts/generate-agreement-content.ts` and commit both, or revert the markdown.",
    ).toEqual({
      version: BCA_F10_VERSION, title: BCA_F10_TITLE,
      subtitle: BCA_F10_SUBTITLE, effectiveNote: BCA_F10_EFFECTIVE_NOTE,
    });
    expect(agreement.preamble).toEqual(BCA_F10_PREAMBLE);
    expect(agreement.sections).toEqual(BCA_F10_SECTIONS);
  });
  ...
  it("the parse is substantial (vacuity tripwire)", () => {
    ...
    expect(BCA_F10_SECTIONS.length).toBeGreaterThan(35);
    expect(BCA_F10_SECTIONS.reduce((n, s) => n + s.clauses.length, 0)).toBeGreaterThan(120);
    expect(BCA_F10_PREAMBLE.length).toBeGreaterThan(2);
    expect(BCA_F10_VERSION).toMatch(/^\d{4}-\d{2}-\d{2}-F\d+$/);
  });
  ...
  it("both tables survived, with their figures", () => {
    ...
    const tabled = BCA_F10_SECTIONS.filter((s) => s.table);
  ...
  it("the execution directive is DROPPED, not rendered as a clause", () => {
    const all = JSON.stringify({ p: BCA_F10_PREAMBLE, s: BCA_F10_SECTIONS });
    expect(all, "the [EXECUTION SECTION ...] directive reached the body").not.toContain("EXECUTION SECTION");
    expect(BCA_F10_SECTIONS.map((s) => s.heading)).not.toContain("Signatures");
  });

  it("the page footer is DROPPED, not repeated as body text", () => {
    const clauses = BCA_F10_SECTIONS.flatMap((s) => s.clauses);
  ...
  it("Schedule A is kept, because it is substantive", () => {
    ...
    const last = BCA_F10_SECTIONS[BCA_F10_SECTIONS.length - 1];
