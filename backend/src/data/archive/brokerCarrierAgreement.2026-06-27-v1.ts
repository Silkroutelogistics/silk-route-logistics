...
//
// The Broker-Carrier Agreement exactly as it stood at version 2026-06-27-v1.
//
// WHY IT IS KEPT. Two carriers executed this text -- AEROSWIFT LLC on
// 2026-09-01 and AMERICAN EAGLE FLEET INC on 2026-09-02 -- and each row
// stores a contentHash computed over it. Only one body per agreement lives in
// the running code, so replacing it with the Foundation Edition would have
// left those hashes un-recomputable: you could no longer demonstrate that the
// stored hash corresponds to the text they signed. The executed PDFs survive
// regardless; what this preserves is the ability to re-derive the evidence.
//
// Version and effectiveNote are LITERALS here, deliberately. They must not
// follow BCA_VERSION when it moves -- following it would change the hash and
// defeat the only reason this file exists.
//
// Resolvable via getAgreement("broker-carrier", "2026-06-27-v1").
// Guarded by __tests__/unit/data/agreementArchive.test.ts.
import type { LegalAgreement } from "../agreements";
export const BROKER_CARRIER_AGREEMENT_2026_06_27_V1: LegalAgreement = {
  ...
  effectiveNote: "Version 2026-06-27-v1 · Effective on execution",
  preamble: [
  ...
  sections: [
    ...
    },
    {
      ...
      clauses: [
        ...
        // FROZEN, not interpolated. The live body resolves this from
        // PAPERWORK_DUE_HOURS so the clause and the Compass grading window
        // cannot drift apart (v3.8.asc). An archive must not track a moving
        // constant: if that value changed, this text would change with it and
        // the two stored hashes would stop re-deriving -- which is the one
        // thing this file exists to prevent. 24 is what it read when these
