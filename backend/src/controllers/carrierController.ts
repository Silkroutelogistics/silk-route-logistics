...
export async function getAllCarriers(req: AuthRequest, res: Response) {
  ...
  const results = await Promise.all(
    carriers.map(async (c) => {
      ...
      // v3.8.awx excluded WITHDRAWN from the denominator here (an offer SRL
      // pulled back was never a chance the carrier had). B3a moves that rule,
      // and the accepted class, into lib/tenderScoring so this surface and the
      // persisted Compass factor cannot disagree again: a carrier who accepted
      // and signed (RC_SENT / CONFIRMED) is accepted here too.
      const tenderSummary = summarizeTenders(c.tenders);
      const tendersAccepted = tenderSummary.accepted;
      const tendersDeclined = tenderSummary.declined;
      const tendersWithdrawn = tenderSummary.withdrawn;
      const tendersTotal = tenderSummary.total;
      ...
      return {
        ...
        tendersWithdrawn,
        tendersTotal,
        // Denominator is offers the carrier could actually act on (judged =
        // total − withdrawn). 0 when nothing was judged — the API shape this
        // surface has always returned; the persisted factor is null there.
        acceptanceRate: tenderSummary.acceptanceRate === null ? 0 : Math.round(tenderSummary.acceptanceRate),
