...
function buildSequenceEmail(template: string, name: string, step: number): string {
  ...
  const bodies: Record<string, string> = {
    introduction: `
      ...
      <p>Compass Engine is our 33-point carrier vetting system. Every carrier on our network clears equipment age, claims ratio, and on-time-in-full at retailer DCs before hauling a load. Real-time GPS tracking on every load via the shipper portal. I personally handle every account today.</p>
