...
| ~~`20260921120000_carrier_profile_fk_restrict`~~ | carrier-archive arc B5, §14 CARRIER ARCHIVE ("Nothing is deleted"); §13.3 Item 291 | **MOVED OUT 2026-09-22 (v3.8.bfw), gate RUN FIRST.** Wasi ran the two header queries against production by hand on 2026-09-22: seven rows, `c` ×5 / `n` ×2, 0 orphans. The file and its seven-line schema hunk then moved into `prisma/migrations/` as one commit (v3.8.bfw), which is where Render applies it. What follows is the record of what the gate was. — Not a drop: seven FKs onto `carrier_profiles` go CASCADE / SET NULL → RESTRICT by DROP + ADD, by name, no `IF EXISTS`. Run the two header queries against production BY HAND (psql / Neon SQL editor): expected exactly seven rows, `c` ×5 and `n` ×2, every orphan count 0. A missing name or a non-zero orphan means production differs from the migration history (Item 273.8 class) — correct the file before it moves. Moves into `prisma/migrations/` together with the seven-line `schema.prisma` hunk listed in its header, as one commit. |
