...
**RESOLVED 2026-08-19 (Arc 6) — deletion, not a gap.** `git log -S "carrierInvoiceId" --all -- backend/src` returns **no commits**: the column has never been referenced in application code at any point in the repository's history. And it is not needed, because the carrier leg reaches exactly-once a different and complete way — `syncCarrierPayAccessorials` compares the ledger total against `CarrierPay.accessorialsTotal` and acts on the delta, re-pricing in place while the settlement is open and escalating a separate payment once it is committed. That is reconciliation by total rather than by row marking. Adding a per-row marker would create a **second** source of truth for "has this been paid", which is the dual-status class this codebase has repeatedly had to unpick. Added to the batched deletion set with the corroboration cited in the migration header.
