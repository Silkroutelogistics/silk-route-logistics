...
| 10 | BOL + shipper notify at CONFIRMED | ✅ **CLOSED v3.8.axv–axw** — the carrier's BOL download requires a CONFIRMED tender (AE exempt), and the shipper fan-out moved to the signature on the DIRECT paths. Auto-dispatch still announces at accept **deliberately** — it dispatches without a signature, so moving it would strand the customer; idempotent on `trackingLinkSent` so a load hitting both triggers announces once | — |
