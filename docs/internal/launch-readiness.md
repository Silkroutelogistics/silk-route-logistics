...
**NEWLY HALTED — the hero-title emphasis device, seven pages.** Trying to prune the italic faces from the font link surfaced a class the guard could not see: `.hero h1 em { font-style: italic }` on `/about`, `/blog`, `/careers`, `/faq`, `/privacy`, `/security-policy` and `/terms`. Those `em`s *inherit* Playfair from each stylesheet's global `h1, h2, h3, h4` rule, so they render Playfair italic while declaring no family of their own — invisible to a check that only reads blocks which declare one. They set the second half of each page title in italic gold ("Information *Security Policy*"). By canon that is drift, since the skill sanctions italic for taglines only; in practice it is a deliberate, consistent device on every legal and content page, and retiring it changes seven heroes. It was not covered by the 2026-08-30 ratification, which named three specific editorial italics. **Halted for Wasi.**

**Consequence: neither italic face was pruned from the Google Fonts link.** The instruction was to prune once zero requests remain, and seven pages still request Playfair italic — at *bold*, since `.hero h1` declares no weight and static pages have no Tailwind preflight to override the UA default. The link stays at `0,400;0,700;1,400;1,500` until this halt resolves. The condition guarded itself, which is the argument for writing prunes as conditionals.
...
| **Hero-title italic `em`** — 7 pages inherit Playfair italic; blocks the font-link prune | Design | Same Oct 2026 hire, or any visual refresh that touches page titles |
