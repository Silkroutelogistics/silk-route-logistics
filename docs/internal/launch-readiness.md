...
**On the four halted heading-weight blocks.** Proof-by-render on the homepage control surfaced a fourth item that source review had passed: `.headline { font-weight: 400 }` is the static section-head class, and the skill puts section heads at 700. Four blocks, three live — `index.css .headline` (the four homepage `<h2>` section heads: "Freight Solutions for Every Lane", "A new trade route, 48 states wide", "The Caravan Partner Program", "Khotan, Our TMS"), `track.css .hero h1`, `track.css .explainer-text h2`. The fourth, `contact.css .headline`, is dead — `contact.html` carries zero `.headline` elements. **Halted, not fixed**, because §20.6 records the homepage editorial register as locked (v3.8.aeq) and `/track` as swept clean (v3.8.amp): the skill says 700, a ratified page-lock has them at 400, and the disagreement is about the weight of the headline on the most-reviewed brand surface there is. Same shape as the shimmer-tagline conflict, same answer. The guard now flags the class with each halt carrying its reason, so it cannot hide again.
...
| **Section heads at Playfair 400** — 3 live blocks on `/` and `/track` | Design (canon conflict) | Same hire, or whenever §20.6 page-locks are revisited |
