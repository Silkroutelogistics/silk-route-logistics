...
**RESOLVED 2026-08-30 (v3.8.avm) — the five italic blocks.** Two are the "Where Trust Travels." shimmer (`.srl-tagline`, `.ops-chip-tagline`). §20.8.2 of CLAUDE.md ratifies that treatment as *"Playfair italic bold 17px with gold gradient"* — a named, reusable pattern. The skill says that exact string is *"Georgia italic, `--gold-dark`, tracked +0.02em"*. **Two project canons disagree about the same sentence.** Resolving that is a product decision. The other three (`.commitment-teaser`, `.tms-result`, `.heritage-line`) are editorial italic lines no sanctioned pattern covers either way. **Ratified in favour of the skill.** Both taglines moved to Georgia italic `--gold-dark` tracked +0.02em, and the shimmer gradient went with them — a spec that names a colour cannot be delivered through `color: transparent`. The three editorial italics moved to roman. §20.8.2 carries the supersession, and the decisive argument is recorded there: that pattern named Playfair 700 italic, **a face neither loader has ever carried**, so what shipped was always a synthesised approximation. Nobody had ever approved its actual render.

**RESOLVED 2026-08-30 (v3.8.avm) — the four heading-weight blocks.** Proof-by-render on the homepage control surfaced a fourth item that source review had passed: `.headline { font-weight: 400 }` is the static section-head class, and the skill puts section heads at 700. Four blocks, three live — `index.css .headline` (the four homepage `<h2>` section heads: "Freight Solutions for Every Lane", "A new trade route, 48 states wide", "The Caravan Partner Program", "Khotan, Our TMS"), `track.css .hero h1`, `track.css .explainer-text h2`. The fourth, `contact.css .headline`, is dead — `contact.html` carries zero `.headline` elements. **Ratified in favour of the skill.** The three live blocks moved to 700 and the dead one was deleted. §20.6 carries the dated supersession with its reasoning: the skill is canon, and the register captured a pre-skill state. The guard's heading allowlist is now empty, which is the correct end state rather than an oversight.

**RESOLVED 2026-08-30 (v3.8.avn) — the hero-title emphasis device, seven pages.** Trying to prune the italic faces from the font link surfaced a class the guard could not see: `.hero h1 em { font-style: italic }` on `/about`, `/blog`, `/careers`, `/faq`, `/privacy`, `/security-policy` and `/terms`. Those `em`s *inherit* Playfair from each stylesheet's global `h1, h2, h3, h4` rule, so they render Playfair italic while declaring no family of their own — invisible to a check that only reads blocks which declare one. They set the second half of each page title in italic gold ("Information *Security Policy*"). By canon that is drift, since the skill sanctions italic for taglines only; in practice it is a deliberate, consistent device on every legal and content page, and retiring it changes seven heroes. **Ratified: roman, gold retained.** The colour was always what carried the emphasis, and the slant was an accident of a missing face — `.hero h1` declares no weight, so the UA default asked for Playfair 700 italic, which neither loader has ever carried. Nobody had approved how it actually rendered.

**Consequence, now discharged: the italic axis is pruned.** The instruction was to prune once zero requests remain; one arc earlier seven pages still requested Playfair italic and the prune was correctly refused. With the device retired, zero elements request it anywhere and the link is `wght@400;700`. The condition guarded itself in both directions, which is the argument for writing prunes as conditionals rather than as steps.
...
| ~~**Section heads at Playfair 400**~~ — RESOLVED v3.8.avm, moved to 700 | — | closed |
| ~~**Hero-title italic `em`**~~ — RESOLVED v3.8.avn, roman + gold; font-link `ital` axis pruned | — | closed |
