        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            ...
            {expanded && (() => {
              ...
              if (!c) return null;
              return (
                <div className="border-t border-white/10 bg-white/[0.02] px-6 py-4 grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      {tab === "lanes" && (
        <div className="space-y-4">
          <select value={laneRegion} onChange={(e) => setLaneRegion(e.target.value)}
            ...
            {(regions ?? []).map((r) => <option key={r.region} value={r.region}>{r.region.replace(/_/g, " ")}</option>)}
          </select>
          <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead>
