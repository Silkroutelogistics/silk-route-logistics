...
export default function MessagesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] gap-0 p-6">
              ...
              <div className="mt-2 space-y-0.5 max-h-40 overflow-y-auto">
                {searchResults.map((u) => (
                  <button key={u.id} onClick={() => selectConversation(u.id, `${u.firstName} ${u.lastName}`)}
                    className="w-full text-left p-2.5 rounded-lg hover:bg-[#1C1F2B] text-sm transition">
      ...
      {/* Message Thread */}
      <div className="flex-1 bg-[#161921] rounded-r-xl border border-[#2A2F42] border-l-0 flex flex-col">
        {selectedUserId ? (
          <>
            {/* Thread Header */}
              ...
              {(!messages || messages.length === 0) && (
                <div className="flex-1 flex items-center justify-center py-16">
                  <div className="text-center">
                    <MessageSquare className="w-8 h-8 text-[#2A2F42] mx-auto mb-2" />
            ...
            {/* Message Input */}
            <div className="px-6 py-4 border-t border-[#2A2F42]">
