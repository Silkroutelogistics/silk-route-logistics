...
function CompassScoreGauge({ score, size = 120 }: { score: number; size?: number }) {
  ...
  const color = score >= 80 ? "#2F7A4F" : score >= 60 ? "#B07A1A" : score >= 40 ? "#B07A1A" : "#9B2C2C";

  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#F5EEE0" strokeWidth={8} />
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={color} strokeWidth={8}
          strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset}
          style={{ transition: "stroke-dashoffset 0.5s ease" }} />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-3xl font-bold text-[#0A2540]">{score}</span>
      </div>
    </div>
  );
}
...
function CompassCategoryBar({ label, passed, total }: { label: string; passed: number; total: number }) {
  ...
  const color = pct >= 80 ? "bg-[#2F7A4F]" : pct >= 60 ? "bg-[#B07A1A]" : "bg-[#9B2C2C]";
  return (
    <div>
      <div className="flex justify-between mb-1">
        <span className="text-xs text-gray-600">{label}</span>
        <span className="text-xs font-bold text-[#0A2540]">{passed}/{total} passed</span>
      </div>
      <div className="bg-[#F5EEE0] rounded h-2 overflow-hidden">
...
function insuranceExpiryColor(dateStr: string | null | undefined): string {
  ...
  if (days < 0) return "text-[#9B2C2C] font-bold";
  if (days <= 30) return "text-[#9B2C2C]";
  if (days <= 60) return "text-[#B07A1A]";
  return "text-[#2F7A4F]";
}
    ...
    <div>
      ...
      {vettingReport && (
        <CarrierCard padding="p-6" className="mb-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-sm font-bold text-[#0A2540]">Your Compass Score</h2>
          </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center gap-3 mt-3">
                <span className={`text-sm font-bold ${
                  vettingReport.grade === "A" ? "text-[#2F7A4F]" :
                  vettingReport.grade === "B" ? "text-[#2A5B8B]" :
                  vettingReport.grade === "C" ? "text-[#B07A1A]" :
                  vettingReport.grade === "D" ? "text-[#B07A1A]" : "text-[#9B2C2C]"
                }`}>Grade {vettingReport.grade}</span>
                <span className="text-gray-500">|</span>
                <span className={`text-sm font-semibold ${
                  vettingReport.riskLevel === "LOW" ? "text-[#2F7A4F]" :
                  vettingReport.riskLevel === "MEDIUM" ? "text-[#B07A1A]" :
                  vettingReport.riskLevel === "HIGH" ? "text-[#B07A1A]" : "text-[#9B2C2C]"
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-4">
                    {vettingReport.checks.map((check, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-xs">
                        {check.result === "PASS" ? (
                          <CheckCircle size={13} className="text-[#2F7A4F] shrink-0" />
                        ) : check.result === "WARNING" ? (
                          <AlertTriangle size={13} className="text-[#B07A1A] shrink-0" />
                        ) : (
                          <AlertCircle size={13} className="text-[#9B2C2C] shrink-0" />
                        )}
                        <span className={`${
                          check.result === "PASS" ? "text-gray-600" :
                          check.result === "WARNING" ? "text-[#B07A1A]" : "text-[#9B2C2C]"
                        }`}>
                          {check.name}
                          {check.result === "WARNING" && (
                            <span className="text-[10px] text-[#B07A1A] ml-1">- Review recommended</span>
                          )}
                        </span>
                      </div>
                    ))}
          ...
          {/* Tip + Last checked */}
          <div className="mt-5 pt-4 border-t border-[#F5EEE0] flex items-center justify-between">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <CarrierCard padding="p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-lg bg-[#2A5B8B]/10 flex items-center justify-center">
        ...
        {/* Insurance */}
        <CarrierCard padding="p-5">
          <h4 className="text-xs font-bold text-[#0A2540] mb-3">Insurance Status</h4>
          {isLoading ? (
            <div className="h-20 bg-[#F5EEE0] rounded animate-pulse" />
          ) : (
            <div className="space-y-2 text-xs">
              <div className="flex justify-between">
        ...
        {/* Documents Status */}
        <CarrierCard padding="p-5">
          <h4 className="text-xs font-bold text-[#0A2540] mb-3">Required Documents</h4>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center justify-between">
                ...
                {d.done ? (
                  ...
                  <AlertCircle size={16} className="text-[#9B2C2C]" />
                )}
              </div>
            ))}
          </div>
      ...
      <CarrierCard padding="p-5" className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-[#0A2540]">Insurance Details</h3>
          {!editingInsurance ? (
            <button onClick={startEditInsurance} className="flex items-center gap-1 px-2.5 py-1 text-xs text-[#BA7517] hover:bg-[#BA7517]/10 rounded-lg transition">
              <Edit3 size={13} /> Edit
            </button>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => updateInsurance.mutate(insForm)} disabled={updateInsurance.isPending}
                className="flex items-center gap-1 px-2.5 py-1 text-xs text-[#FBF7F0] bg-[#BA7517] hover:bg-[#BA7517]/90 rounded-lg transition disabled:opacity-50">
        ...
        {!editingInsurance ? (
          <div className="space-y-3">
            ...
            {[
              ...
              <div key={line.label} className="flex items-center justify-between py-2 border-b border-[#F5EEE0] last:border-0">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-[#0A2540] w-28">{line.label}</span>
            ...
            <div className="flex items-center gap-4 pt-2">
              <span className={`text-xs flex items-center gap-1 ${insurance?.additionalInsuredSRL ? "text-[#2F7A4F]" : "text-gray-400"}`}>
                {insurance?.additionalInsuredSRL ? <CheckCircle size={13} /> : <AlertCircle size={13} />} Additional Insured
              </span>
              <span className={`text-xs flex items-center gap-1 ${insurance?.waiverOfSubrogation ? "text-[#2F7A4F]" : "text-gray-400"}`}>
                {insurance?.waiverOfSubrogation ? <CheckCircle size={13} /> : <AlertCircle size={13} />} Waiver of Subrogation
              </span>
              <span className={`text-xs flex items-center gap-1 ${insurance?.thirtyDayCancellationNotice ? "text-[#2F7A4F]" : "text-gray-400"}`}>
          ...
          <div className="space-y-4">
            ...
            {[
              ...
              <div key={line.label}>
                <p className="text-xs font-semibold text-[#0A2540] mb-1.5">{line.label}</p>
                <div className="grid grid-cols-3 gap-2">
                  <input
                    ...
                    className="px-2.5 py-1.5 border border-[#EFE6D3] rounded-lg text-xs focus:ring-1 focus:ring-[#BA7517]/15 focus:border-[#BA7517] outline-none"
                  />
                  <input
                    placeholder="Policy #"
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <CarrierCard padding="p-5">
          <h3 className="text-sm font-bold text-[#0A2540] mb-4">FMCSA BASIC Scores</h3>
          {Object.keys(basicScores).length === 0 ? (
            ...
            <div className="space-y-3">
              {Object.entries(basicScores).map(([key, value]) => {
                ...
                const color = score <= 50 ? "bg-[#2F7A4F]" : score <= 75 ? "bg-[#B07A1A]" : "bg-[#9B2C2C]";
                return (
                  <div key={key}>
                    <div className="flex justify-between mb-1">
        ...
        <CarrierCard padding="p-5">
          <h3 className="text-sm font-bold text-[#0A2540] mb-4">SRL Performance Metrics</h3>
          {srlMetrics ? (
            <div className="space-y-3">
              {[
                ...
                const color = v >= 90 ? "bg-[#2F7A4F]" : v >= 75 ? "bg-[#B07A1A]" : "bg-[#9B2C2C]";
                return (
                  <div key={m.label}>
                    <div className="flex justify-between mb-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <CarrierCard padding="p-5">
          <h3 className="text-sm font-bold text-[#0A2540] mb-4">Upcoming Expirations</h3>
          {expItems.length === 0 ? (
            <div className="py-6 text-center text-xs text-gray-700">No upcoming expirations</div>
          ) : (
            <div className="space-y-2">
              {expItems.map((exp: any, i: number) => (
                <div key={i} className="flex justify-between items-center py-2 border-b border-[#F5EEE0]">
                  <div className="flex items-center gap-2">
                    <Clock size={14} className={exp.status === "EXPIRED" ? "text-[#9B2C2C]" : exp.status === "EXPIRING_SOON" ? "text-[#B07A1A]" : "text-gray-400"} />
        ...
        <CarrierCard padding="p-5">
          <h3 className="text-sm font-bold text-[#0A2540] mb-4">Compliance Alerts</h3>
          {alerts.length === 0 ? (
            <div className="py-6 text-center text-xs text-[#2F7A4F] flex flex-col items-center gap-2">
              <CheckCircle size={24} />
            ...
            <div className="space-y-2">
              {alerts.map((alert: any, i: number) => (
                <div key={i} className={`p-3 rounded-md text-xs ${
                  alert.severity === "critical" ? "bg-[#F6E3E3] border border-[#9B2C2C]/30 text-[#9B2C2C]" :
                  alert.severity === "warning" ? "bg-[#FBEFD4] border border-[#B07A1A]/30 text-[#B07A1A]" :
                  "bg-[#E2EAF2] border border-[#2A5B8B]/30 text-[#2A5B8B]"
                }`}>
