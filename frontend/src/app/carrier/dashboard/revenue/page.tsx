  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="font-serif text-2xl text-[#0D1B2A] mb-1">Revenue &amp; Earnings</h1>
          ...
          {(["weekly", "monthly", "ytd"] as Period[]).map((p) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-[11px] font-semibold uppercase tracking-wider transition-all ${
                p === period ? "bg-white text-[#0D1B2A] shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          <CarrierCard key={kpi.label} padding="p-5">
            ...
            <div className="text-[28px] font-bold text-[#0D1B2A]">{kpi.value}</div>
          </CarrierCard>
        ))}
      </div>
      ...
      <CarrierCard className="mb-6" padding="p-5">
        <h2 className="text-sm font-semibold text-[#0D1B2A] mb-4">Revenue Over Time</h2>
        {chartData.length === 0 ? (
        ...
        <div className="col-span-2">
          <CarrierCard padding="p-0">
            <div className="px-4 py-3 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-[#0D1B2A]">Revenue Breakdown</h2>
            </div>
                ...
                <tbody>
                  {invoices.length === 0 ? (
                    ...
                    <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-2.5 font-mono text-[11px] font-semibold text-[#0D1B2A]">{inv.load?.referenceNumber || "—"}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-600">
                        {inv.load ? `${inv.load.originCity}, ${inv.load.originState} → ${inv.load.destCity}, ${inv.load.destState}` : "—"}
                      </td>
                      <td className="px-4 py-2.5 font-bold text-[#0D1B2A]">${(inv.amount || 0).toLocaleString()}</td>
        ...
        <div className="space-y-4">
          ...
          <CarrierCard padding="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-semibold text-[#0D1B2A]">Top Lanes</h2>
            </div>
            {topLanes.length === 0 ? (
              ...
              <div key={i} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                <div>
                  <div className="text-[12px] font-semibold text-[#0D1B2A]">{lane.lane}</div>
          ...
          <CarrierCard padding="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-semibold text-[#0D1B2A]">Payment Summary</h2>
            </div>
            {[
