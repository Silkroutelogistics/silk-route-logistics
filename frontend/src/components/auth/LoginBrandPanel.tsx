...
const VARIANTS: Record<LoginVariant, VariantConfig> = {
  ae: {
    ...
    slides: [
      ...
      { label: "COMPASS COMPLIANCE", text: "33-check carrier vetting built for safety first" },
  ...
  carrier: {
    ...
    slides: [
      ...
      // "QuickPay Express" was a retired tier name, "24 hours" matched no
      // published Quick Pay speed, and "no paperwork" contradicted both the
      // agreement the carrier signs and the document-timeliness factor they are
      // graded on. Restated from the honest claim in CLAUDE.md §4 #3.
      ...
      <div className="relative z-10 flex flex-col items-start w-full max-w-[480px] px-12">
        ...
        <div className="grid grid-cols-2 gap-2 mb-8 w-full max-w-[360px]">
          {featurePills.map((f) => (
            <div
              key={f.label}
              className="flex items-center justify-center gap-1.5 border border-[#FFFFFF] text-[#0A2540] bg-[#FFFFFF] rounded-lg px-3 py-2.5 text-xs font-medium cursor-default"
            >
