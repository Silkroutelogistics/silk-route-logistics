...
export function CarrierSidebar() {
  ...
  return (
    <>
      {/* Mobile header bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-[#0D1B2A] border-b border-white/5 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-md bg-gradient-to-br from-[#C9A84C] to-[#A88535] flex items-center justify-center text-[#0D1B2A] text-[10px] font-black tracking-tight">
            SR
          </div>
      ...
      {/* Sidebar */}
      <aside
        className={`w-[220px] bg-[#0D1B2A] border-r border-white/5 flex flex-col h-screen flex-shrink-0 transition-transform duration-200 fixed lg:sticky top-0 z-50 lg:z-auto ${
