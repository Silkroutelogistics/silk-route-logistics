/**
 * wall-icons.js — Inline SVG icon library for Section 5 capabilities wall.
 *
 * v3.8.agd — Replaces the Lucide CDN dependency. Root cause of icons
 * missing on the deployed page: `https://unpkg.com/lucide@latest` was
 * resolving to lucide@1.16.0 (the archived original package), not the
 * modern Lucide. None of the icon names referenced existed in that old
 * library, so `lucide.createIcons()` silently did nothing.
 *
 * This module replaces that approach entirely. All 26 icons used by the
 * capabilities-wall are inlined as Lucide-style stroke SVGs. Color
 * inherits from CSS `color`/`stroke: currentColor`, so the brand-
 * canonical gold-dark / navy treatment per tile data-cat continues to
 * work. Zero CDN race, zero version drift.
 *
 * Icons drawn at 24x24 viewBox, stroke-width=2 (CSS overrides to 1.85),
 * stroke-linecap=round, stroke-linejoin=round. Style matches Lucide's
 * stroke conventions.
 */
(function () {
  var WALL_ICONS = {
    'thermometer-snowflake': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 4v10.54a4 4 0 1 1-4 0V4a2 2 0 0 1 4 0Z"/><line x1="2" y1="12" x2="6" y2="12"/><line x1="4" y1="10" x2="4" y2="14"/></svg>',
    'banknote': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01"/><path d="M18 12h.01"/></svg>',
    ...
    'scroll-text': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 17V5a2 2 0 0 0-2-2H4"/><path d="M8 21h11a2 2 0 0 0 2-2v-1a1 1 0 0 0-1-1H9"/><path d="M3 5a2 2 0 0 0 2 2h3a1 1 0 0 0 1-1V3"/><path d="M15 8h-5"/><path d="M15 12h-5"/></svg>',
    'smartphone': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>',
    ...
    'calendar-clock': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 7.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h3.5"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="8" y2="10"/><circle cx="16" cy="16" r="6"/><path d="M16 14v2l1.5 1"/></svg>',
    ...
    'snowflake': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="2" y1="12" x2="22" y2="12"/><line x1="12" y1="2" x2="12" y2="22"/><path d="m20 16-4-4 4-4"/><path d="m4 8 4 4-4 4"/><path d="m16 4-4 4-4-4"/><path d="m8 20 4-4 4 4"/></svg>',
    ...
    'phone-call': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.79 19.79 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.72 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.35 1.85.59 2.81.72A2 2 0 0 1 22 16.92z"/><path d="M14.05 2a9 9 0 0 1 8 7.94"/><path d="M14.05 6A5 5 0 0 1 18 10"/></svg>',
    'gavel': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m14 13-7.5 7.5c-.83.83-2.17.83-3 0a2.12 2.12 0 0 1 0-3L11 10"/><path d="m16 16 6-6"/><path d="m8 8 6-6"/><path d="m9 7 8 8"/><path d="m21 11-8-8"/></svg>',
  ...
  /**
   * paintWallIcons(rootEl) — find all <i data-lucide="name"> elements
   * inside rootEl and replace each with the corresponding inline SVG
   * from WALL_ICONS. Safe to call multiple times — only converts
   * elements that still have the data-lucide attribute (replaced
   * <svg> elements no longer match).
   */
  function paintWallIcons(rootEl) {
    if (!rootEl) return;
    var nodes = rootEl.querySelectorAll('[data-lucide]');
    for (var i = 0; i < nodes.length; i++) {
      var name = nodes[i].getAttribute('data-lucide');
      var svgString = WALL_ICONS[name];
      if (!svgString) continue;
      var tmp = document.createElement('div');
      tmp.innerHTML = svgString.trim();
      var svg = tmp.firstChild;
      if (svg && nodes[i].parentNode) {
        nodes[i].parentNode.replaceChild(svg, nodes[i]);
      }
    }
  }

  window.WALL_ICONS = WALL_ICONS;
  window.paintWallIcons = paintWallIcons;
})();
