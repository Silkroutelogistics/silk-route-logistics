...
42. **RC Modal Generate PDF + Send Tender buttons fail silently — root cause logged OPEN, Sprint 33 close.** Surfaced 2026-05-06 during Sprint 28 Phase B Tier 1.2 walk continuation. Both buttons render no UI feedback when clicked — no toast, no alert, no inline error, no success notification. Pre-Sprint-32 the catch blocks at [`RateConfirmationModal.tsx:563`](frontend/src/components/loads/RateConfirmationModal.tsx#L563), [`:582`](frontend/src/components/loads/RateConfirmationModal.tsx#L582), [`:550`](frontend/src/components/loads/RateConfirmationModal.tsx#L550) all swallowed errors via `console.error("...", err)` with no surfaced UI feedback — making remote diagnosis impossible without DevTools Network inspection. **Sprint 32 ships Item 43 (error UI surfacing) FIRST** — the next Generate PDF / Send Tender click after deploy will display the actual API error in a red banner above the button row. **Sprint 33 closes Item 42** once the surfaced error message identifies the root cause. **Most likely cause** (educated guess pending diagnosis): Zod validator at [`backend/src/validators/rateConfirmation.ts:16-200`](backend/src/validators/rateConfirmation.ts#L16) rejecting one of ~50 form-state fields spread via `...form` in [`buildPayload()`](frontend/src/components/loads/RateConfirmationModal.tsx#L510). Schema is permissive (`.optional()` on most fields) but a type mismatch (e.g., empty string where `z.number().optional()` expected, or wrong enum value) throws ZodError → 500/400 to client → swallowed before Sprint 32. Could also be auth (modal user role mismatch with `authorize("BROKER", "ADMIN", ...)`) or backend internal error. **Fix shape unknown** until Sprint 33 diagnosis — likely 1-15 LOC schema relaxation OR frontend payload sanitization. Bug class: "validator strict, frontend payload promiscuous" — long-term resolution is Sprint 28 Phase A Item A28-2 (validator tightening with shape coercion). For Sprint 33 just fix whatever specific field rejects.
