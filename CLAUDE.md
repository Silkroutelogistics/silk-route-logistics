...
156. ~~**Carrier route RSC payload regression.**~~ **CLOSED 2026-05-17 in v3.8.abx** (Sprint 52.hotfix.a). `_redirects` rule `/carrier/* /carrier/:splat 200` echoed `:splat` as identity rewrite target. Cloudflare Pages resolved the extensionless `/carrier/login` ambiguously between `out/carrier/login.html` and `out/carrier/login.txt` siblings (Next.js static export generates both — `.html` for browser navigation, `.txt` for client-side RSC streaming). Gmail iOS WebView Accept-header content negotiation landed on the `.txt` RSC payload, displaying raw `1:"$Sreact.fragment"` Flight payload instead of the rendered carrier login page. Production INTEGRITY EXPRESS carrier reported the regression after clicking the Sprint 45a tender email link. **Fix:** replaced the `:splat` echo with explicit `.html` rewrites for 4 top-level carrier routes (`/carrier/login`, `/carrier/forgot-password`, `/carrier/reset-password`, `/carrier/dashboard`) + 1 wildcard for dashboard sub-routes (`/carrier/dashboard/* → /carrier/dashboard/:splat.html`). Deterministic resolution; matches `/verify/*` (Sprint 51) and `/track/*` (Sprint 26 Item 31) single-target precedent. Legacy static HTML files in `public/carrier/` (`analytics`, `help`, `loads`, `register`, `tools`) resolve via Cloudflare default extension fallback — no rule needed because no `.txt` sibling exists for those files.
