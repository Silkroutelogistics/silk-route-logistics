...
88. **Inline body styles in 12 emailService.ts transactional functions hardcode off-skill colors outside `wrap()`.** Surfaced 2026-05-10 during Sprint 45a Sub-phase 0 enumeration. `wrap()` was reconciled to skill canonical in Sprint 45a, but the 12 transactional functions that USE wrap() still inline drift in their body strings: h2 `#0f172a` (Tailwind slate-900, → should be `#0A2540` --navy), CTA buttons `#d4a574` (off-canonical olive, → should be `#BA7517` --gold-dark for emphasis), text muted `#64748b/#94a3b8` (slate, → should be `#3A4A5F` --fg-2 / `#6B7685` --fg-3), card border `#e2e8f0` (slate-200, → should be `#E2EAF2` --navy-100), status colors `#dc2626/#22c55e/#f59e0b/#3b82f6` (Tailwind, → should be skill `--success #2F7A4F / --warning #B07A1A / --danger #9B2C2C / --info #2A5B8B`). Affected functions: `sendPreTracingEmail`, `sendAutoInvoiceEmail`, `sendLateAlertEmail`, `sendOtpEmail`, `sendQpVarianceReport`, `sendPasswordResetEmail`, `sendRateConfirmationEmail`, `sendRiskAlertEmail`, `sendFallOffAlertEmail`, `sendRemittanceEmail`, `sendPasswordExpiryReminder`, plus `shipperPickupHtml/TransitHtml/DeliveryHtml/PODHtml` helpers. Sprint 45a's new tender email functions (`sendTenderOfferedEmail` etc.) were authored to skill canonical from the start — they don't drift. **Sprint shape:** Sprint 47+ docs-only-style mass-find-replace inside `emailService.ts`, ~30-50 LOC across 12 functions. Pairs with Item 87 as a brand-chrome reconciliation duo. Not BKN-blocking — visual polish only; emails currently render fine in all major clients.
