...
```
npm install && npm run build && npx prisma migrate deploy && npx prisma migrate status && cp -r src/assets dist/backend/src/assets && cp -r src/config dist/backend/src/config
```
...
48. **RC PDF format does not match `srl-brand-design` skill canonical (BOL v2.9 reference).** Surfaced 2026-05-07 during Sprint 34 Phase C smoke when user shared rendered RC PDF (`RC-L7492033667.pdf`). Multiple format issues vs skill canonical: (1) **6 pages with phantom-blank pages 2/3/5/6** — page-break logic produces empty pages with just header/footer chrome; content actually fits on 2 pages cleanly. (2) **Address duplicated** in header chrome — renders `"2317 S 35th St, Galesburg, MI 49053"` then `"Galesburg, MI 49053"` on next line. Template populates both `address` and `city, state, zip` fields redundantly. (3) **No QR code** — BOL v2.9 has `/track/<token>` QR per [`utils/qrGenerator.ts`](backend/src/utils/qrGenerator.ts); RC has none. RC should also include QR for carrier scan-and-confirm UX. (4) **No canonical Compass mark** — generic small compass icon shown; BOL v2.9 has the canonical Compass mark per `srl-brand-design` skill scripts. (5) **Carrier Information section renders empty (`—`)** even when carrier selected post-Sprint-32 dropdown fix — field-binding gap in [`pdfService.ts/generateEnhancedRateConfirmation`](backend/src/services/pdfService.ts) doesn't pull `carrierCompany`/`carrierMC`/`carrierDOT`/`carrierContact`/`carrierPhone` from the saved rate confirmation `formData`. (6) **TOTAL section bare** — shows `Line Haul Rate: $3,750 / Driver assist: $250 / TOTAL: $4,000` but no Caravan tier breakdown, no QP fee detail, no payment terms reflection of Sprint 33 Caravan tier reconciliation. (7) **Font/section-header chrome drift** from skill canonical — different fonts, different section header treatment, different brand color tokens than BOL v2.9. **Sprint shape:** multi-day architectural sprint post-BKN, scope ~200-400 LOC across `pdfService.ts` + `srl-brand-design` skill template integration. Parallels BOL v2.9 epic shape (per CLAUDE.md §11 v3.8.a → v3.8.d). **NOT BKN-blocking** — current PDF is functional (data is correct after Sprint 30 Houston-fix); just doesn't match skill polish. Pairs naturally with §13.3 Item 8.9 (central authority module) since both touch customer-facing PDF generation surfaces.
