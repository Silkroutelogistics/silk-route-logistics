...
```
npm install && npm run build && npx prisma migrate deploy && npx prisma migrate status --exit-code && cp -r src/assets dist/backend/src/assets && cp -r src/config dist/backend/src/config
```

The `prisma migrate status --exit-code` step is a post-deploy verification gate (Sprint 44b P3). It fails the build if any migrations remain pending after `migrate deploy` runs, catching silent-skip class regressions before the new artifact ships.
