...
75. **E2E coverage gap on FRONTEND tender create flow.** Surfaced 2026-05-09 during Sprint 44c hotfix retrospective. Existing E2E B6.5 family (B6.5/B6.5a/B6.5b/B6.5c/B6.5d/B6.5e per [`e2e/full-lifecycle.spec.ts`](e2e/full-lifecycle.spec.ts)) tests backend API directly with singular canonical at lines 150, 158, 225. Frontend modal drift to plural was never caught because no E2E exercises modal click → frontend POST → 404 path. The B6.5 create flow uses `request.post(${BACKEND_API}/loads/${load.id}/tender, ...)` — bypasses the frontend's `api.post()` wrapper entirely. **Sprint shape:** Sprint 46+ candidate. New B6.5f playwright test that loads the tender modal in a browser context, selects a carrier via the picker, clicks "Send Tender", asserts the network request URL goes to canonical singular `/api/loads/${id}/tender` (regression-locks Sprint 44c fix), asserts response 201, asserts load transitions to BOOKED. ~50-80 LOC. Pairs with §13.3 Item 66 (E2E shipper-portal auth fixture + ShipmentDetailDrawer a11y regression lock + T&T navigation regression — same E2E coverage-extension methodology debt class).
