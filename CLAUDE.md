...
42. **RC Modal Generate PDF + Send Tender buttons fail silently — extractor widening shipped Sprint 33 (v3.8.aal); targeted field fix awaits Sprint 34.** Surfaced 2026-05-06 during Sprint 28 Phase B Tier 1.2 walk continuation. Sprint 32 (v3.8.aak) shipped error-UI surfacing — banner appeared but only showed generic "Validation error" because [`extractErrorMessage`](frontend/src/components/loads/RateConfirmationModal.tsx) only checked top-level `data.error` / `data.message`, never iterated the `data.details[]` array Zod errors return per [`backend/src/middleware/errorHandler.ts:66-76`](backend/src/middleware/errorHandler.ts#L66). **Sprint 33 (v3.8.aal) widened the extractor** to iterate `data.details` when present, joining `field: message` pairs into a readable string. Banner now reads e.g. `"Validation error — formData.fuelSurcharge: Expected number, received undefined"` — surfaces the specific Zod field rejection. **Path β methodology (continued from Sprint 32):** observability ships first, targeted fix ships subsequent atomic commit. **Sprint 34 closes Item 42** with the targeted field fix once next click attempt surfaces the specific rejection. Likely cause (educated guess pending diagnosis): one of ~50 form-state fields spread via `...form` in [`buildPayload()`](frontend/src/components/loads/RateConfirmationModal.tsx) has type mismatch (empty string where `z.number().optional()` expected, or wrong enum value, or shape mismatch on accessorials/stops/dates). Could also be auth or backend internal error. **Fix shape unknown** until next click surfaces it — likely 1-15 LOC schema relaxation OR frontend payload sanitization. Long-term resolution is Sprint 28 Phase A Item A28-2 (validator tightening with shape coercion).
