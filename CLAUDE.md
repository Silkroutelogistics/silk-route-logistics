...
```
npm install && npm run build && npx prisma migrate deploy && npx prisma migrate status && cp -r src/assets dist/backend/src/assets && cp -r src/config dist/backend/src/config && cp -r src/lib dist/backend/src/lib
```
...
71. **`(npx tsc || true)` suppresses TypeScript errors in build chain.** Surfaced 2026-05-09 during Sprint 44b Phase 3 audit of `backend/package.json` build script. The `build` script reads `npx prisma generate && (npx tsc || true)` — the `|| true` shell idiom swallows the TypeScript compiler's exit code on errors, allowing the build to "succeed" even when `tsc` reports type errors. Sprint 44b's new Render Build Command uses `npm run build` (which inherits this suppression) followed by `prisma migrate deploy && prisma migrate status --exit-code`. Net effect: type errors in production code can ship without surfacing in the build output. **Risk profile:** type errors that cross runtime boundaries (e.g., undefined property access, wrong argument shape) would surface as runtime errors instead of compile-time ones. CI workflow runs `npx tsc --noEmit` separately at the lint step (`.github/workflows/ci.yml:32`), which is stricter — prod build inherits whatever main last passed CI, so the gap is narrow but real. **Sprint shape:** remove `|| true` from `backend/package.json` build script. Verify a clean tsc on current main pre-removal so the change doesn't immediately fail prod builds. ~1 LOC fix + verification. **Priority:** medium — latent class of runtime errors that should surface earlier. Sprint 45+ candidate. Pairs with Item 8.10 closure (deploy chain reliability theme).
