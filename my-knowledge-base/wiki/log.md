...
## [2026-04-08] ingest | 5 GitHub repos analyzed for SRL improvements
Extracted and analyzed: claw-code (Rust agent harness), claude-code (leaked Anthropic CLI source), claude-brain (Go multi-agent framework), claw-codes (Python+Rust rewrite), src-repo (Claude Code TypeScript core). Adopted 2 patterns immediately: state machine lifecycle definitions + lane-based development charter added to CLAUDE.md (rules 10-12). Documented 3 future patterns: hook system (Q2), cost tracker split (Q2), feature flags (if Bun migration). Previous 3 repos (GBrain, Graphify, Hermes) already integrated in KB v2 schema.
